...
pump_mode = 0  # 0=q, 1=k, 2=p
delta_vals = np.linspace(0.0, 2*np.pi, 101)  # phase difference between pump modes

cases = [(0, 1), (1, 0), (1, 1)]
case_names = ['HM-like', 'Only pressure advection','ITG w.o. diamagnetic']
...
print(f"M={0.5 * cross}, Lqkp={Lqkp_base}, Lkpq={Lkpq_base}, Lpqk={Lpqk_base}")
...
for case_idx, (zero_m, zero_l) in enumerate(cases):
    M = zero_m * 0.5 * cross
    ...
    if pump_mode==0:
        tr = np.abs(Phi0)**2*(Lkpq*Lpqk - M**2) 
        det = -M**2*np.abs(Phi0)**2*(Lkpq*Phi0.conj())*(Lpqk*Phi0)
    elif pump_mode==1:
        tr = np.abs(Phi0)**2*(Lqkp*Lpqk - M**2) 
        det = -M**2*np.abs(Phi0)**2*(Lqkp*Phi0.conj())*(Lpqk*Phi0)
    elif pump_mode==2:
        tr = np.abs(Phi0)**2*(Lqkp*Lkpq - M**2) 
        det = -M**2*np.abs(Phi0)**2*(Lqkp*Phi0.conj())*(Lkpq*Phi0)
    
    print(f"Case {case_idx+1}/{len(cases)}: det={det}")
    tr_cases.append(tr)
    det_cases.append(det)
...
for i, (zero_m, zero_l) in enumerate(cases):
    ...
    plt.plot(
        ...
        lam_vals[i],
        'o-',
        label=f"{m_label}, {l_label}: {case_names[i]}",
    )
