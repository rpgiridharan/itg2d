...
plt.loglog(k[1:-1], P2k[k1]*k[1:-1]**(-3), 'k--', label = '$k^{-3}$')
plt.loglog(k[1:-1], P2k[k1]*k[1:-1]**(-4), 'r--', label = '$k^{-4}$')
plt.xlabel('$k$')
...
plt.figure(figsize=(16, 9))
plt.loglog(k[1:-1], V2k[1:-1], label = r'$\left|V_{k}\right|^2$')
plt.loglog(k[V2k_ZF>0][1:-1], V2k_ZF[V2k_ZF>0][1:-1], label = r'$\left|V_{k,\mathrm{ZF}}\right|^2$')
plt.loglog(k[1:-1], V2k_turb[1:-1], label = r'$\left|V_{k,\mathrm{turb}}\right|^2$')
plt.loglog(k[1:-1], V2k[k1]*k[1:-1]**(-2), 'k--', label = '$k^{-2}$')
plt.loglog(k[1:-1], V2k[k1]*k[1:-1]**(-3), 'r--', label = '$k^{-3}$')
plt.xlabel('$k$')
...
plt.tight_layout()
if fname.endswith('out.h5'):
    plt.savefig(datadir+'parallel_velocity_spectrum.pdf', dpi=100)
else:
    plt.savefig(datadir+"parallel_velocity_spectrum_" + fname.split('/')[-1].split('out_')[-1].replace('.h5', '.pdf'), dpi=100)
plt.show()

plt.figure(figsize=(16, 9))
plt.loglog(k[1:-1], Ek[1:-1], label = r'$E_{k}$')
plt.loglog(k[Ek_ZF>0][1:-1], Ek_ZF[Ek_ZF>0][1:-1], label = r'$E_{k,\mathrm{ZF}}$')
plt.loglog(k[1:-1], Ek_turb[1:-1], label = r'$E_{k,\mathrm{turb}}$')
plt.loglog(k[1:-1], Ek[k1]*k[1:-1]**(-5/3), 'r--', label = r'$k^{-5/3}$')
plt.loglog(k[1:-1], Ek[k1]*k[1:-1]**(-3), 'k--', label = r'$k^{-3}$')
plt.xlabel('$k$')
plt.ylabel(r'$E_k$')
plt.legend()
plt.grid(which='both', linestyle='--', linewidth=0.5)
...
# plt.loglog(k[1:-1], Kk[k1]*k[1:-1]**(-3), 'r--', label = r'$k^{-3}$')
# plt.loglog(k[1:-1], Kk[k1]*k[1:-1]**(-5), 'k--', label = r'$k^{-5}$')
# plt.xlabel(r'$k$')
...
# plt.figure(figsize=(16, 9))
# plt.loglog(k[1:-1], Wk[1:-1], label = r'$W_{k}$')
# plt.loglog(k[Wk_ZF>0][1:-1], Wk_ZF[Wk_ZF>0][1:-1], label = r'$W_{k,\mathrm{ZF}}$')
# plt.loglog(k[1:-1], Wk_turb[1:-1], label = r'$W_{k,\mathrm{turb}}$')
# plt.loglog(k[1:-1], Wk[k1]*k[1:-1]**(1/3), 'k--', label = r'$k^{1/3}$')
# plt.loglog(k[1:-1], Wk[k1]*k[1:-1]**(-1), 'r--', label = r'$k^{-1}$')
# plt.xlabel(r'$k$')
# plt.ylabel(r'$W_k$')
# plt.legend()
# plt.grid(which='both', linestyle='--', linewidth=0.5)
...
plt.figure(figsize=(16, 9))
plt.loglog(k[1:-1], Gk[1:-1], label = r'$G_{k}$')
plt.loglog(k[Gk_ZF>0][1:-1], Gk_ZF[Gk_ZF>0][1:-1], label = r'$G_{k,\mathrm{ZF}}$')
plt.loglog(k[1:-1], Gk_turb[1:-1], label = r'$G_{k,\mathrm{turb}}$')
plt.loglog(k[1:-1], Gk[k1]*k[1:-1]**(-3), 'r--', label = r'$k^{-3}$')
plt.loglog(k[1:-1], Gk[k1]*k[1:-1]**(-5), 'k--', label = r'$k^{-5}$')
plt.xlabel(r'$k$')
plt.ylabel(r'$G_{k}$')
plt.legend()
plt.grid(which='both', linestyle='--', linewidth=0.5)
