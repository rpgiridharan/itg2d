...
for case_idx, (zero_m, zero_l, zero_n) in enumerate(cases):
    ...
    if pump_mode==0:
        ...
        det = -M**2*np.abs(Phi0)**2*(Lkpq*Phi0.conj() + Nkpq*P0.conj()*(kdp-qdk))*(Lpqk*Phi0 - Npqk*P0*(kdp-pdq))
    elif pump_mode==1:
        ...
        det = -M**2*np.abs(Phi0)**2*(Lqkp*Phi0.conj() + Nqkp*P0.conj()*(qdk - pdq))*(Lpqk*Phi0 - Npqk*P0*(kdp - pdq))
    elif pump_mode==2:
        ...
        det = -M**2*np.abs(Phi0)**2*(Lqkp*Phi0.conj() + Nqkp*P0.conj()*(pdq - qdk))*(Lkpq*Phi0 - Nkpq*P0*(kdp - qdk))
