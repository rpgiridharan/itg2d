...
comm.Barrier()
datadir = 'data_2d3c/'
fname = datadir+'out_2d3c_kapt_2_0_D_0_03_kz_0_1_1024x1024.h5'
...
if rank == 0:
    ...
    plt.figure(figsize=(16, 9))
    plt.semilogy(t[:nt], V2_t, label = '$V_{total}$')
    plt.semilogy(t[:nt], V2_ZF_t, label = '$V_{ZF}^2$')
    plt.semilogy(t[:nt], V2_turb_t, label = '$V_{turb}^2$')
    plt.xlabel('$\\gamma t$')
    plt.ylabel('$V^2$')
    plt.title('$V^2$ vs $\\gamma$ t')
    plt.grid()
    plt.legend()
    plt.tight_layout()
    if fname.endswith('out.h5'):
